...
def testMany(netG, keypointsPath, imagesPath, outputPath, outputSubpath, imageExtension, saveImages=True):
    ...
    normalizer = normalization.NormalizationHeatmaps(outputRes=HEATMAP_SIZE, sigma=2)
    ...
    for filename in jsonFiles:
        ...
        try:
            ...
            #print("keypoints_cropped_norm[0]:", keypoints_cropped_norm[0])
            ...
            keypoints_cropped_norm = torch.tensor(keypoints_cropped_norm)
            confidence_values = torch.tensor(confidence_values)
            #keypoints_cropped = keypoints_cropped.flatten()
            batch_of_one_keypoints_cropped.append(keypoints_cropped_norm)
            ...
            print("WARNING: skipping "+ filename+" error="+str(e))
            traceback.print_exc()
            pass
    ...
    #print("stacking: ", batch_of_one_keypoints_cropped)
    ...
    fakeReshapedAsKeypoints = normalization.denormalizeBatch(fake.cpu().numpy(), batch_scaleFactor, batch_x_displacement, batch_y_displacement)
    ...
    for idx in range(len(fakeReshapedAsKeypoints)):
        ...
        #fakeKeypointsCroppedOneImageIntRescaled = normalization.denormalize(fakeKeypointsOneImage, batch_scaleFactor[idx], batch_x_displacement[idx], batch_y_displacement[idx])
        fakeKeypointsCroppedOneImageIntRescaled=fakeKeypointsOneImage
